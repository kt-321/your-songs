<ul class="nav nav-tabs nav-justified mb-3">
    ...
    <li class="nav-item"><a href="{{ route("users.timeline", ["id" => $user->id]) }}" class="nav-link {{ Request::is("users/*/timeline") ? "active" : ""}}"><i class="far fa-clock mr-2"></i>タイムライン<span class="badge badge-secondary ml-1"> {{ $count_feed_songs }}</span></a></li>
    @else
    ...
    <li class="nav-item"><a href="{{ route("users.favorites", ["id" => $user->id])}}" class="nav-link {{ Request::is("users/*/favorites") ? "active" : ""}}"><i class="fas fa-star mr-2"></i>お気に入り <span class="badge badge-secondary ml-1">{{ $count_favorites }}</span></a></li>
</ul>