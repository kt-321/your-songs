...
class UploadSongImageRequestTest extends TestCase
{
    ...
    {   
        ...
        //画像をアップロードする
        $response = $this->actingAs($user)->post(route("songs.songImagesUpload", ["id" => $song->id]),[
            "file" => $uploadedFile,
        ]);
    ...
    {   
        ...
        //画像をアップロードする
        $response = $this->actingAs($user)->from(route("songs.songImages", ["id" => $song->id]))->post(route("songs.songImagesUpload", ["id" => $song->id]),[
            "file" => "",
        ]);
        
        // アップロードに失敗し、曲画像のアップロード画面が表示される
        $response->assertStatus(302);
        $response->assertRedirect(route("songs.songImages", ["id" => $song->id]));
